...
class XDefinitionCRUD:
    ...
    def update_one(self, definition_id, **kwargs):
        ...
        if not fields:
            ...
            return False
        sql = f"UPDATE Definition SET {', '.join(fields)} WHERE id = %s"
    ...
    def delete_one(self, definition_id):
        sql = 'DELETE FROM Definition WHERE id = %s'
        try:
            cursor = self.conn.cursor()
            cursor.execute(sql, (definition_id,))
            self.conn.commit()
    ...
    def read_by_pos_code(self, pos):
        sql = '''SELECT * FROM Definition WHERE pos_code = %s'''
        try:
            cursor = self.conn.cursor(dictionary=True)
    ...
    def read_by_definition_like(self, like):
        sql = '''SELECT * FROM Definition WHERE definition LIKE %s'''
        try:
            cursor = self.conn.cursor(dictionary=True)
